...
<body>
    ...
    <form action="{{ route('kost.update', $kost->id) }}" method="POST" enctype="multipart/form-data" class="w-full mx-5">
        ...
        <div class="mb-6">
            ...
            <input type="text" id="nama_kost" name="nama_kost" value="{{ $kost->nama_kost }}"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />
        </div> 
    
        <div class="mb-6">
            <label for="deskripsi_kost" class="block mb-2 text-sm font-medium text-gray-900">Deskripsi</label>
            <textarea id="deskripsi_kost" name="deskripsi_kost" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>{{ $kost->deskripsi_kost }}</textarea>
        </div> 
    
        <div class="mb-6">
            <label for="peraturan_kost" class="block mb-2 text-sm font-medium text-gray-900">Peraturan Kost</label>
            <textarea id="peraturan_kost" name="peraturan_kost" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>{{ $kost->peraturan_kost }}
            </textarea>
        </div> 
        
        <div class="mb-6">
            <label for="fasilitas_kost" class="block mb-2 text-sm font-medium text-gray-900">Fasilitas</label>
            <textarea id="fasilitas_kost" name="fasilitas_kost" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>{{ $kost->fasilitas_kost }}
            </textarea>
        </div> 
        ...
        <div class="grid gap-6 mb-6 md:grid-cols-2">
            ...
            <div>
                ...
                <input type="tel" id="no_telepon" name="no_telepon" 
                       ...
                       value="{{ $kost->no_telepon }}"
                />
            </div>            
            <div>
                <label for="harga" class="block mb-2 text-sm font-medium text-gray-900">Harga</label>
                <input type="text" id="harga" name="harga" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required value="{{ $kost->harga }}"/>
            </div>  
            <div>
                <label for="jumlah_kamar" class="block mb-2 text-sm font-medium text-gray-900">Jumlah Kamar</label>
                <input type="number" id="jumlah_kamar" name="jumlah_kamar" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required value="{{ $kost->jumlah_kamar }}" />
            </div>
        </div>
        <div class="mb-6">
            ...
            <!-- Tempat untuk menampilkan semua gambar (lama dan baru) -->
            <div id="images-preview" class="mt-4 grid grid-cols-2 md:grid-cols-3 gap-4">
                <!-- Loop untuk menampilkan gambar lama -->
                @foreach($kost->images as $image)
                    <div class="relative" id="image-{{ $image->id }}">
                        <img src="{{ asset('images/' . $image->image) }}" class="object-cover h-48 w-full rounded-lg" alt="{{ $kost->nama_kost }}">
                        <!-- Tombol hapus untuk gambar lama -->
                        <button type="button" class="absolute top-1 right-1 bg-red-500 text-white rounded-full px-2 py-1 text-xs" onclick="deleteOldImage('{{ $image->id }}')">
                            Hapus
                        </button>
                    </div>
                @endforeach
